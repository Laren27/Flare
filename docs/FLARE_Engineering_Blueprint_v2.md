...
**ADR-022: WebSocket authentication**
- Decision: `WS /ws/{user_id}` accepts the connection, then requires the client's first frame to be `{"type": "auth", "token": "<jwt>"}` within 5 seconds. The token is verified with the same code path as HTTP requests, and its `sub` must equal the `user_id` in the path or the socket is closed with 1008. Anything other than a valid auth frame closes the connection.
- Alternatives considered: `?token=...` in the handshake query string; a short-lived single-use ticket issued over HTTP and redeemed on connect; trusting the path `user_id` as Chapter 14 literally specifies; cookie-based session auth.
- Why: the endpoint as written in Chapter 14 has no authentication at all — any party could open `/ws/7` and receive user 7's alerts, which in this system means the location and description of somebody's medical emergency. That is the most sensitive data the product handles, so it cannot be left to a path parameter. Browsers cannot set an `Authorization` header on a WebSocket handshake, which rules out the obvious fix and is why the query-string idiom is common — but a bearer token in a URL is written to server access logs, proxy logs, and browser history, and a JWT valid for an hour sitting in a log file is a credential leak with a delay fuse. A ticket endpoint solves that properly and is the right production answer; it costs an extra table or cache and an extra round trip, which is not justified at demo scale (Rule 002, Rule 004). First-frame auth keeps the credential in the message body where it belongs, reuses the existing token verification unchanged, and costs one `await` and a timeout.
- Note: the path parameter is retained rather than dropped, so the endpoint still reads as Chapter 14 specifies. It is treated as an assertion the client makes and the server checks, not as identity.

**ADR-023: Incident History records the initiating escalation trigger**
- Decision: `IncidentHistory.escalation_trigger` stores the condition that *started* an incident escalating, not the condition in effect at the final rung.
- Context: a single incident can escalate under both ADR-012 conditions in sequence. A realistic run observed during week 4: wave 1 at 1km found nobody (condition A, immediate), wave 2 at 2km alerted three responders who stayed silent (condition B, after the timeout), wave 3 at 3km likewise, then the ladder ran out.
- Alternatives considered: recording the last trigger before termination; recording a list of every trigger fired.
- Why: ADR-015 uses this field for exactly one purpose — separating *too few responders* from *unresponsive responders*, because those have entirely different remedies. Recording the last trigger destroys that: the final rung almost always finds nobody new, so nearly every incident would be filed as `empty_set`, including the ones where responders were alerted and ignored the alert. That is the opposite diagnosis, and it would point recruitment at areas whose real problem is engagement. A list of triggers would be strictly more information but needs a column type the schema does not have, for a metric defined in ADR-015 as an A-versus-B split; the first trigger is the one that answers the question asked.
- Note: the per-wave detail is not lost. `DispatchEvents` carries `wave_number` and `radius_m_at_eval` on every row, so the full escalation trail is reconstructable by query — this field is a summary, not the record.

**ADR-024: AI summary transport and provider**
- Decision: the single bounded call of ADR-005 is a plain HTTPS POST written against the provider's REST API, not a vendor SDK. The provider is selected by one environment variable (`AI_PROVIDER`, `gemini` or `groq`); the default is Google AI Studio with `gemini-3.5-flash-lite`. The API key travels in a request header, never a query string. Absent a key, the service reports `skipped` and dispatch proceeds unchanged.
- Alternatives considered: the `google-genai` SDK; an OpenAI-compatible client library; automatic failover from Gemini to Groq on quota exhaustion.
...
**ADR-027: Incident updates are pushed, and carry a snapshot**
- Decision: the socket of ADR-022 gains server-to-client incident events. To the **victim**: `sos_matched`, `sos_escalated`, `sos_resolved`, `sos_cancelled`, `sos_no_responder`. To every **volunteer still holding an open alert**: `alert_closed`, with the reason. Every incident event carries the same payload — a full status snapshot of the incident, the same fields as `GET /sos/{id}` — rather than a delta. The client reconciles with one `GET /sos/{id}` on connect and on every reconnect, and does not poll on an interval.
- Context: Chapter 13 step 9 specifies push and rejects polling explicitly, "since the socket already exists". The citizen client polled every two seconds anyway, because no event existed to listen for. The sharper problem was on the other side: a volunteer holding an alert for an incident somebody else had already accepted was told nothing, and discovered it only by pressing ACCEPT and losing.
- Alternatives considered: keeping the poll; per-field deltas; a separate event type per changed field; server-sent events alongside the WebSocket.
- Why: a snapshot rather than a delta because the socket has no replay. A client that was disconnected when an event fired cannot reconstruct state from a diff it never received, so every delta design needs a resync path anyway — and once that exists, the delta saves nothing but costs a second code path that is exercised rarely and therefore wrong. A snapshot is idempotent: applying the same event twice, or applying only the last of five, lands in the same place. Distinct event *names* are kept even though the payload is uniform, because the citizen view genuinely reacts differently — an escalation must not clear the map, an acceptance must — and branching on a name reads better than branching on a status field.
- On the reconciliation fetch: one `GET /sos/{id}` when the socket opens is not polling. It is the resync a stateless socket requires, it runs once per connection rather than every two seconds, and without it a dropped socket silently freezes the citizen's screen mid-emergency — which is the failure this ADR exists to remove, not one to introduce.
- Note: `alert_closed` is delivery-best-effort like every other frame. A volunteer whose socket is down when an incident is taken still learns on their next accept attempt, which is the path that already existed. The event removes the wait, it does not replace the guarantee.

**ADR-026: Availability carries a position**
- Decision: `PATCH /volunteers/availability` takes `{available, lat, lng}`. Going **online** requires coordinates and the server refuses the request without them (422); going **offline** takes `available: false` alone. The same call upserts the volunteer's `Locations` row, and the volunteer client re-sends it on a timer while online, which is the Chapter 17 location polling.
- Context: availability and location are coupled by the dispatch engine and were not coupled anywhere else. ADR-021 rejects a verified, available volunteer with no `Locations` row as `no_location`, so a volunteer could switch themselves "Online", be told they were visible to people nearby, and never be a candidate for anything. Until now the only writer of `Locations` was `sim/seed.py`, so every browser-registered volunteer was in exactly that state.
- Alternatives considered: a separate `PUT /volunteers/location` with availability left independent; availability alone, with location left to a later feature; a background location poll that runs regardless of availability.
- Why: separating the two endpoints is the more conventional REST shape and was rejected because it makes the coupling a matter of client discipline — any client that forgets the second call produces a volunteer who is online and undispatchable, and the server cannot tell that this has happened. Folding the position into the availability call moves the invariant to the API boundary, where "online" cannot be recorded without the system knowing where. Availability alone would have shipped exactly the control this redesign spent its first phase removing: one that looks like it works and silently does not. Polling location regardless of availability would track people who have declared themselves off duty, which is a privacy cost taken for no dispatch benefit — an offline volunteer is never a candidate.
- Note: this does not add an endpoint to Chapter 14; it specifies the body of one already listed there. `POST /volunteers/register` remains unbuilt — skills are set at signup and editing them is not yet reachable.
...
- v3.1 — ADR-027 (incident updates pushed over the existing socket, carrying a status
  snapshot rather than a delta, with a reconciliation fetch on connect). Resolves the
  standing contradiction between Chapter 13 step 9, which rejects polling, and a citizen
  client that polled every two seconds because no event existed to listen for. Also
  closes the gap where a volunteer holding an alert for an already-accepted incident
  was told nothing until they pressed ACCEPT and lost.
- v3.0 — ADR-026 (availability carries a position). The volunteers router of Ch. 14
  is built for the first time, with `PATCH /volunteers/availability` refusing to mark
  a volunteer online without coordinates — closing the gap where a browser-registered
  volunteer could be "Online" and permanently undispatchable under ADR-021's
  `no_location`.
